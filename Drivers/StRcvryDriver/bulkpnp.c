...
Abstract:
    ...
    Bulk USB device driver for SigmaTel STMP3410/3500/3600 Recovery-mode devices Plug and Play module.
...
/*
	ntStatus = IoCreateSymbolicLink(&deviceExtension->SymbolicLinkName, &deviceExtension->DeviceName);
    if(!NT_SUCCESS(ntStatus)) {

		BulkUsb_DbgPrint(1, ("IoCreateSymbolicLink::create:failed.(0x%x)\n device:%ws\n link:  %ws", ntStatus, deviceExtension->DeviceName.Buffer, deviceExtension->SymbolicLinkName.Buffer));
...
--*/
{
	...
	{
        BulkUsb_DbgPrint(1, ("ConfigureDevice() failed. deviceExtension->UsbConfigurationDescriptor should be NULL\n"));
...
--*/
{
	...
	Globals.Stmp3RecInstances[deviceExtension->Instance] = INSTANCE_FREE;

	ntStatus = IoDeleteSymbolicLink(&deviceExtension->SymbolicLinkName);
    if(!NT_SUCCESS(ntStatus)) {

        BulkUsb_DbgPrint(1, ("IoDeleteSymbolicLink::delete:failed\n"));
    }
