...
// SGTL - Modify start
	// SGTL Recovery-device only has write pipe, "PIPE00"
	UNICODE_STRING substitutePipeName;
	RtlInitUnicodeString(&substitutePipeName, L"PIPE00");
// SGTL - Modify end
    PAGED_CODE();
...
*/
	pipeContext = BulkUsb_PipeWithName(DeviceObject, &substitutePipeName);
// SGTL Modify End
    if(pipeContext == NULL) {
...
BulkUsb_DispatchCreate_Exit:
    ...
    BulkUsb_DbgPrint(3, ("BulkUsb_DispatchCreate - ends status=%x\n", ntStatus));
