...
//      doesn't have this.
	...
	#define SS_IS_INTRESOURCE(_r) IS_INTRESOURCE(_r)
#else
...
#include <vector>           // for vectors of CStdString
...
// _bstr_t
	...
	#pragma warning (push)
	#pragma warning (disable : 6386) // Buffer Overrun warning ignored in MS header file
	#include <comdef.h>
	#pragma warning (pop)
...
// of the Standard C++ Library, you can define your own version of SS_USE_FACET.
    ...
    #define schMSG(desc) message(__TFILE__ "(" schSTR2(__LINE__) "):" #desc)
#endif
...
inline PWSTR StdCodeCvt([Pre(Null=No)] PWSTR pDstW, int nDst, [Pre(Null=No)] PCSTR pSrcA, int nSrc,
...
inline PSTR StdCodeCvt([Pre(Null=No)]PSTR pDstA, int nDst, [Pre(Null=No)]PCWSTR pSrcW, int nSrc,
    const std::locale& loc=std::locale())
{
    ASSERT(0 != pDstA);
    ASSERT(0 != pSrcW);
...
// Promote them to the global namespace so we can use them here.
    ...
    inline int  ssnprintf(PSTR pA, size_t sizeA, size_t nCount, PCSTR pFmtA, va_list vl)
    { 
        return _vsnprintf_s(pA, sizeA, nCount, pFmtA, vl);
    }
    inline int  ssnprintf(PWSTR pW, size_t sizeA, size_t nCount, PCWSTR pFmtW, va_list vl)
    {
        return _vsnwprintf_s(pW, sizeA, nCount, pFmtW, vl);
    }
...
public:
    ...
    #define SSRES(pctstr)  LOWORD(/*clw reinterpret_cast<unsigned long>*/(pctstr))  
...
#if defined(SS_WIN32) && !defined(SS_ANSI)
        if ( ( pT != NULL ) && SS_IS_INTRESOURCE((ULONG_PTR)pT) )
        {
            UINT nId = LOWORD(/*clw reinterpret_cast<unsigned long>*/(pT));
            if ( !LoadString(nId) )
            {
...
//      }
    ...
    #ifdef SS_ANSI
        int nLen = ssvsprintf(szBuf, STD_BUF_SIZE, STD_BUF_SIZE-1, szFmt, argList);
    #else
        int nLen = ssnprintf(szBuf, STD_BUF_SIZE, STD_BUF_SIZE-1, szFmt, argList);
    #endif
        if ( 0 < nLen )
    ...
    void FormatV(const CT* szFormat, va_list argList)
    {
        ...
        do  
        {
			...
			nUsed           = ssnprintf(pBuf, nChars, nChars-1, szFormat, argList);
...
#if 0 // TODO: clw - don't know why this breaks in MFC
#ifdef _MFC_VER
...
#endif
	typedef std::vector<CStdString> StdStringArray;
