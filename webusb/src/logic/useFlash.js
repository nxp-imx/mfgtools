...
async function send_data(USBDevice, data, success_str) {
    send_output(USBDevice, data);
    get_reply(USBDevice, success_str);
}
...
async function send_chunk_begin (USBDevice, chunk_len, i) {
    let hex_len = (chunk_len + 52).toString(16);

    await send_data(USBDevice, str_to_arr(`download:${hex_len}`), `DATA${hex_len}`);
    await send_headers(USBDevice, chunk_len, i);
}
...
export async function process_chunk (USBDevice, chunk, i) {
    ...
    // pad chunk with zeros to the nearest block size
