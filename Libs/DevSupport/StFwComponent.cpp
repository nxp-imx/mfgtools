...
StFwComponent::StFwComponent(LPCTSTR fileName, LoadFlag loadFlag, uint16_t langId)
: _fileName(fileName)
, _id(InvalidId)
, _langId(langId)
, _startingOffset(0)
...
const uint32_t StFwComponent::size() const
{
    if ( !_data.empty() && ((_data.size() - _startingOffset) > 0) )
        return (uint32_t)_data.size() - _startingOffset;
    else
        return 0;
}
...
const CStdString StFwComponent::GetShortFileName() const
{
	TCHAR fname[_MAX_FNAME];
	TCHAR ext[_MAX_EXT];
	CStdString shortName;

	_tsplitpath_s(_fileName.c_str(), NULL, 0, NULL, 0, fname, _MAX_FNAME, ext,_MAX_EXT);
...
/*
const size_t StFwComponent::GetSizeInSectors(const uint32_t sectorSize) const
{
	if( sectorSize )
	{
		return 0;
...
const ParameterT<StFwComponent::FileType>& StFwComponent::GetFileType()
{
    ...
    uint32_t index = (uint32_t)fileTag35.find(FileTypeTag_Stmp);
    if ( index != std::string::npos )
    {
...
void StFwComponent::GetData(size_t fromOffset, size_t count, uint8_t * pDest) const
{
    // 3500-style files have 2 lines of plain text defining the ProductVersion and ComponentVersion.
...
const uint8_t * const StFwComponent::GetDataPtr() const
{ 
    return _data.empty() ? NULL : &_data[0] + _startingOffset; 
}

int32_t StFwComponent::Load(LPCTSTR fileName, LoadFlag loadFlag, uint16_t langId)
{
	switch (loadFlag)
    {
...
int32_t StFwComponent::LoadFromFile(LPCTSTR fileName)
{
    clear();
    ...
    // Get the file size
    uint32_t fileSize = file.rdbuf()->pubseekoff(0, std::ios::end, std::ios::in);
	...
	if ( file.rdbuf()->_Sgetn_s((char*)&_data[0], _data.size(), fileSize) != fileSize )
	{
		file.close();
		return ERROR_INVALID_DATA;
	}
...
// modifies member variables _fileName, _data, _startingOffset, _fileDate, and _fileType
int32_t StFwComponent::LoadFromResource(LPCTSTR resourceName, uint16_t langId)
{
    clear();
	...
	_langId = langId;
    _isResource = true;
	int32_t iResId = GetResourceID(resourceName);
	...
	uint8_t * pResourceData = (uint8_t*)LockResource(hRes);
    if ( pResourceData == NULL )
        return ERROR_LOCK_FAILED;

    // Get the file size
    uint32_t resourceSize = SizeofResource(GetModuleHandle(NULL), hResInfo);

    // Size our vector to accomodate the file size.
    _data.resize(resourceSize);

	// Put the data from the file in our vector
    if ( memcpy_s((uint8_t*)&_data[0], _data.size(), pResourceData, resourceSize) != ERROR_SUCCESS )
...
const CStdString StFwComponent::toString() const
{
    CStdString theInfo;
...
int32_t StFwComponent::GetResourceID(LPCTSTR _fname)
{
	int32_t resId = IDR_RESID_UNKNOWN;;
	LPCTSTR pFileName;
