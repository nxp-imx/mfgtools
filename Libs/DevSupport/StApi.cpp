...
int32_t StApi::SetCommandData(const uint8_t * const pData, const size_t size)
{
...
//    size_t arraySize = max(size,_xferLength);
    ...
    if ( size && pData )
    {
        _sendDataPtr = (uint8_t*)malloc(size);
...
StApi* StApiFactory::CreateApi(CStdString name, CStdString paramStr)
{
...
bool StApiFactory::RegisterApi(CStdString name, CreateApiCallback createFn)
{
	return _callbacks.insert(CallbackMap::value_type(name, createFn)).second;
};

bool StApiFactory::UnregisterApi(CStdString name)
{
