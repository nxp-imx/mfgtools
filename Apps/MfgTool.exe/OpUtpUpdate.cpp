...
//	m_sVersion = GetProjectVersion();
	...
	if ( m_UclNode.Load(A2T(uclString)) != NULL )
	{
		...
		// Configure VolumeDeviceClass with the UCL::DeviceState::Updater vid and pid values
		if( m_DeviceStates.find(UCL::DeviceState::Updater) != m_DeviceStates.end() )
		{
			TCHAR *p;
			gDeviceManager::Instance()[DeviceClass::DeviceTypeMsc]->SetMSCVidPid((USHORT)_tcstoul(m_DeviceStates[UCL::DeviceState::Updater]->GetVid(),&p,16),
				(USHORT)_tcstoul(m_DeviceStates[UCL::DeviceState::Updater]->GetPid(),&p,16));
		}
		else
		{
			//Set an invalid value
			gDeviceManager::Instance()[DeviceClass::DeviceTypeMsc]->SetMSCVidPid(0x00, 0x00);
		}
		if( m_DeviceStates.find(UCL::DeviceState::Finished) != m_DeviceStates.end() )
		{
			TCHAR *p;
			gDeviceManager::Instance()[DeviceClass::DeviceTypeCst]->AddFilter((USHORT)_tcstoul(m_DeviceStates[UCL::DeviceState::Finished]->GetVid(),&p,16),
				(USHORT)_tcstoul(m_DeviceStates[UCL::DeviceState::Finished]->GetPid(),&p,16));
		}
	}
...
const UCL::DeviceState::DeviceState_t COpUtpUpdate::GetDeviceState()
{
	...
	switch ( (DeviceClass::DeviceType)m_pUSBPort->GetDeviceType() )
	{
		...
		case DeviceClass::DeviceTypeCst:
			state = UCL::DeviceState::Finished;
			break;
	...
	ATLTRACE(_T("%s device state is changed to  %d \r\n"),m_pPortMgrDlg->GetPanel(), state);
	return state;
}
...
///    OverallProgressBar.Value = 0;
	...
	pOperation->m_OpState = COpUtpUpdate::OP_READY;
...
{
    DWORD retValue = 0;
	//Must set m_OpState here in case any unexpected state, say OP_INVALID, set in previous operation break the function.
	m_OpState = WAITING_FOR_DEVICE;
...
///        Thread.CurrentThread.Name = "DoBoot";
	...
	*/
	...
	if(((DeviceClass::DeviceType)m_pUSBPort->GetDeviceType() == DeviceClass::DeviceTypeMxRom) || 
		((DeviceClass::DeviceType)m_pUSBPort->GetDeviceType() == DeviceClass::DeviceTypeMxHid)
		)
	{
		//retValue = DoMxRomLoad(pCmd);
...
DWORD COpUtpUpdate::DoPlugin(UCL::Command* pCmd)
{
    ...
    if((DeviceClass::DeviceType)m_pUSBPort->GetDeviceType() == DeviceClass::DeviceTypeMxRom)
    {
    	...
    	if( !pMxRomDevice->RunPlugIn(fullFileName) ) 
    	{
    		returnVal = ERROR_INVALID_HANDLE;
    		ATLTRACE(_T("!!!ERROR!!! %s Failed to initialize i.MxRom memory. OpState: %s\r\n"), m_pPortMgrDlg->GetPanel(), GetOpStateString(m_OpState));
    		return returnVal;
    	}
    }
    else
