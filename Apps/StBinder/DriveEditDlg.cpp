...
CDriveEditDlg::CDriveEditDlg(CWnd* pParent /*=NULL*/, BOOL _bNew, int _index)
	: CDialog(CDriveEditDlg::IDD, pParent)
{
	m_Parent = (CTargetDriveArrayDlg *)pParent;
	m_drive_array = m_Parent->m_drive_array;
	m_bIsNewDrive = _bNew;
...
{
    DDX_Text(pDX, IDC_DRIVE_EDIT_DESCRIPTION, m_str_edit_description);
    DDX_Control(pDX, IDC_DRIVE_EDIT_COMBO_DRIVE_TYPE, m_combo_drive_type);
    DDX_Control(pDX, IDC_DRIVE_EDIT_COMBO_DRIVE_TAG, m_combo_drive_tag);
    DDX_Text(pDX, IDC_DRIVE_EDIT_REQUESTED_DRIVE_SIZE, m_dw_edit_requested_drive_size);
    DDV_MinMaxUInt(pDX, m_dw_edit_requested_drive_size, 0, 4294967295);
    DDX_Control(pDX, IDC_DRIVE_EDIT_DESCRIPTION, m_ctrl_edit_description);
    DDX_Control(pDX, IDC_DRIVE_EDIT_REQUESTED_DRIVE_SIZE, m_ctrl_edit_requested_drive_size);
...
BEGIN_MESSAGE_MAP(CDriveEditDlg, CDialog)
	...
	ON_CBN_SELCHANGE(IDC_DRIVE_EDIT_COMBO_DRIVE_TYPE, &CDriveEditDlg::OnCbnSelchangeComboDriveType)
	ON_CBN_SELCHANGE(IDC_DRIVE_EDIT_COMBO_DRIVE_TAG, &CDriveEditDlg::OnCbnSelchangeComboDriveTag)
END_MESSAGE_MAP()
...
BOOL CDriveEditDlg::OnInitDialog() 
{
	...
	GetDlgItem(IDC_DRIVE_EDIT_FILE_NAME)->EnableWindow(TRUE);
	m_ctrl_edit_requested_drive_size.EnableWindow(TRUE);

	m_ctrl_edit_file_name.SubclassDlgItem(IDC_DRIVE_EDIT_FILE_NAME, this);
	...
	if( m_index != -1 )
	{
		media::LogicalDrive driveDesc = m_drive_array[m_index];
		m_original_filename = driveDesc.Name;
...
void CDriveEditDlg::SetDlgItems(const media::LogicalDrive& _driveDesc)
{
	...
	if( _driveDesc.Type != media::DriveType_Invalid )
	{
		...
		{	// hidden (Janus) and data drives cannot have files associated
		    GetDlgItem(IDC_DRIVE_EDIT_FILE_NAME)->EnableWindow(FALSE);
		}
		else
		{
			CString itemStr;
			...
			itemStr = _driveDesc.Name;
			if (itemStr)
				SetDlgItemText(IDC_DRIVE_EDIT_FILE_NAME, itemStr);
		}
		m_ctrl_edit_description.EnableWindow(TRUE);
		m_ctrl_edit_requested_drive_size.EnableWindow(TRUE);
	}
...
BOOL CDriveEditDlg::GetDlgItems(media::LogicalDrive& _driveDesc)
{
	...
	{	// hidden (Janus) and data drives cannot have files associated
		...
		GetDlgItemText(IDC_DRIVE_EDIT_FILE_NAME, csFname);
...
{
	if ( GetDlgItems( m_drive_array[m_index] ) )
	{
		m_Parent->m_drive_array[m_index] = m_drive_array[m_index];
//		return;
	}
...
//	m_combo_drive_type.SetItemData();
	...
	if( m_index != -1 )
	{
		switch (m_drive_array[m_index].Type)
		{
...
void CDriveEditDlg::InitDriveTagCombo()
{
	...
	for( int i = 0; i < MAX_TAGS; ++i)
	{
		...
		for( j = 0; j < (int) m_drive_array.Size(); ++j )
		{
			if( m_drive_array[j].Tag == g_TagList[i].tag )
			{
	...
	if ( g_TagList[m_combo_drive_tag.GetItemData(m_combo_drive_tag.GetCurSel())].tag == media::DriveTag_DataSettings ||
		g_TagList[m_combo_drive_tag.GetItemData(m_combo_drive_tag.GetCurSel())].tag == media::DriveTag_Data2 )
	{
		GetDlgItem(IDC_DRIVE_EDIT_FILE_NAME)->EnableWindow(TRUE);
	}
...
BOOL CDriveEditDlg::CheckForPrerequisite(media::LogicalDriveTag _tag)
{
	...
	// check that a prerequisite tagged drive exists
	for( z = 0; z < (int) m_drive_array.Size(); ++z)
	{
		if( m_drive_array[z].Tag == _tag )
			break;
	}

	if( z == m_drive_array.Size() )
...
BOOL CDriveEditDlg::CheckForDependents()
{
	...
	{
		if( g_TagList[z].prerequisite == m_drive_array[m_index].Tag )
		{
	...
	if( z < MAX_TAGS )
	{
		for( z = 0; z < (int) m_drive_array.Size(); ++z )
		{
			if( m_drive_array[z].Tag == dependentTag )
			{
				bDependency = TRUE;
				break;
			}
...
void CDriveEditDlg::Localize()
{
	...
	resStr.LoadStringW(IDS_DRIVE_EDIT_DIALOG_TITLE);
	SetWindowText(resStr);

	resStr.LoadString(IDS_DRIVE_EDIT_FNAME_TEXT);
	SetDlgItemText(IDC_DRIVE_EDIT_FNAME_TEXT, resStr);

	resStr.LoadString(IDS_DRIVE_EDIT_DESC_TEXT);
	SetDlgItemText(IDC_DRIVE_EDIT_DESC_TEXT, resStr);

	resStr.LoadString(IDS_DRIVE_EDIT_DRIVE_TYPE_TEXT);
	SetDlgItemText(IDC_DRIVE_EDIT_DRIVE_TYPE_TEXT, resStr);

	resStr.LoadString(IDS_DRIVE_EDIT_DRIVE_TAG_TEXT);
	SetDlgItemText(IDC_DRIVE_EDIT_DRIVE_TAG_TEXT, resStr);

	resStr.LoadString(IDS_DRIVE_EDIT_DRIVE_SIZE_TEXT);
	SetDlgItemText(IDC_DRIVE_EDIT_DRIVE_SIZE_TEXT, resStr);
